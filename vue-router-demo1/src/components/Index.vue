<template>
	...
	<div class="container">
		<div class="top">
			...
			<router-link to="/newCourse"><label>新建班课</label></router-link>
		</div>
		<hr>
			...
			<div class="course" v-for="(course, index) in coursesI" :key="index">
				...
				<div class="tearcher">
					<div class="left">
						...
						<div class="use-username">
							{{course.username}}
						</div>
					</div>
					<div class="course-code" v-if="course.userId===loginUserId">
						{{ course.courseCode }}
					</div>
				</div>
			</div>
...
export default {
	...
	created() {
		var _this = this;
		this.$http.get('http://localhost:8080/api/coursesI').then(function(response) {
			_this.coursesI = response.data;
		});
		this.$http.get('http://localhost:8080/api/coursesF').then(function(response) {
